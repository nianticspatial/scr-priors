...
datasets_folder="${current_dir}/datasets/scannet/scans_test/"
# output directory for the reconstruction
out_dir="${current_dir}/reconstructions/scannet/ace0_rgbd_laplace_nll"
# target directory for benchmarking results
benchmarking_out_dir="${current_dir}/benchmark/scannet/ace0_rgbd_laplace_nll"
# directory with split files for the non-standard 60/60 split
split_file_dir="${current_dir}/splits"

# get list of scenes, we only use the first 20 test scenes in our experiments
scene_list=$(ls ${datasets_folder} | head -20)
...
for scene in ${scene_list}; do
  ...
  input_rgb_files="${datasets_folder}/${scene}/sensor_data/*.color.jpg"
  input_depth_files="${datasets_folder}/${scene}/sensor_data/*.pgm"
  ...
  # get focal length
  focal_path="${datasets_folder}/${scene}/sensor_data/_info.txt"
  # read focal length file and get the first line
  focal_length=$(awk '/m_calibrationColorIntrinsic/ {print $3}' ${focal_path})

  echo "Using focal length from Scannet: ${focal_length}"
  ...
  (cd "${ace_zero_path}" && python $reconstruction_exe "${input_rgb_files}" \
                                                       ...
                                                       ${export_pc_cmd} \
                                                       --depth_files "${input_depth_files}" \
                                                       --depth_use_always True \
                                                       --loss_structure probabilistic \
                                                       --prior_loss_type rgbd_laplace_nll \
                                                       --prior_loss_weight 1.0 \
                                                       --prior_loss_bandwidth 0.1 \
                                                       ) 2>&1 | tee "${scene_out_dir}/log_${scene}.txt"

  # run pose evaluation
  (cd "${ace_zero_path}" && python eval_poses.py "${scene_out_dir}/poses_final.txt" \
                                                 "${datasets_folder}/${scene}/sensor_data/*.pose.txt" \
  ...
  if $run_benchmark; then
    ...
    # run the standard PSNR benchmark split with 60 query frames and 60 mapping frames
    benchmarking_scene_dir="${benchmarking_out_dir}/60_60/${scene}"
    mkdir -p ${benchmarking_scene_dir}
    (cd "${ace_zero_path}" && conda run --no-capture-output -n ${benchmarking_environment} python -m benchmarks.benchmark_poses --pose_file "${scene_out_dir}/poses_final.txt" \
                                                                                                                                --output_dir "${benchmarking_scene_dir}" \
                                                                                                                                --images_glob_pattern "${input_rgb_files}" \
                                                                                                                                --method "${benchmarking_method}" \
                                                                                                                                --crop 15 \
                                                                                                                                --split_json "${split_file_dir}/${scene}.json" \
                                                                                                                                ) 2>&1 | tee "${benchmarking_out_dir}/log_${scene}.txt"
  fi
done
...
echo "60-60 Split"
echo "----------------------------------------------------"
(cd "${ace_zero_path}" && python scripts/show_benchmark_results.py "${benchmarking_out_dir}/60_60" --method ${benchmarking_method})
echo "===================================================="
fi