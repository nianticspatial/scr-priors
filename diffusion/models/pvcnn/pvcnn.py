...
from diffusion.models.pvcnn.modules import Attention
from diffusion.models.pvcnn.pvcnn_utils import create_mlp_components, create_pointnet2_sa_components, create_pointnet2_fp_modules, get_timestep_embedding
...
class PVCNN2Base(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, beta: torch.Tensor, t: torch.Tensor, context: torch.Tensor = None, permute_axis: bool = True):
        """
        ...
        if context is not None:
            context = context.permute(0, 2, 1).contiguous()  # (B, S, N)
            inputs = torch.cat([inputs, context], dim=1)  # (B, 3 + S, N)
